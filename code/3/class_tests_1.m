...
%         wald_orig_test_stat
        ...
        beta2_hat_full_Taylor, beta2_hat_pars_Taylor
        Wald_Taylor, Max_Taylor, Max_t_Taylor
        theta_hat_full_Taylor_bs, theta_hat_pars_Taylor_bs
        beta2_hat_full_Taylor_bs, beta2_hat_pars_Taylor_bs
    ...
    methods
        ...
        function obj = class_tests_1(test_number, dgp_type, hypothesis_type, n, seed_input, delta_in, beta2_in, pi_in, k_beta2_n)
            obj@class_dgp_1(test_number, dgp_type, hypothesis_type, n, seed_input, delta_in, beta2_in, pi_in);
            obj.k_beta2_n = k_beta2_n;
        end
        function obj = set_params(obj, k_beta2_n)
        ...
        function obj = fcn_tests_initial(obj)
            Y = obj.Y0;
            X = [obj.x_delta; obj.x_beta2; obj.x_pi];
%             obj.Ns = [sqrt(obj.n) .* ones(obj.k_delta+obj.k_beta2,1); ones(1+obj.k_beta2,1)];
            obj.Ns_full = sqrt(obj.n) .* [ones(obj.k_delta+obj.k_beta2,1); abs(obj.theta_hat_full(obj.k_delta:obj.k_delta+obj.k_beta2,1))];
            obj.Ns_full_lambda = obj.Ns_full(obj.ind_lambda_full);
            obj.Ns_pars = ones(obj.k_delta+3, obj.k_beta2);
            obj.Ns_pars(obj.k_delta+1+1:obj.k_delta+1+1+1,:) = abs(obj.theta_hat_pars(obj.k_delta:obj.k_delta+1,:));
            obj.Ns_pars = sqrt(obj.n) .* obj.Ns_pars;
            obj.Ns_pars_lambda = obj.Ns_pars(obj.ind_lambda_pars,:)';
            
            obj.lambda_hat_full = obj.theta_hat_full(obj.ind_lambda_full);
            obj.lambda_hat_pars = obj.theta_hat_pars(obj.ind_lambda_pars, :)';
            ...
            % subtracting beta2_0 for initial observations only
%             obj.wald_Cheng_test_stat = obj.n * (obj.beta2_hat_full - obj.beta2_0)' * inv(V_hat_full_beta2) * (obj.beta2_hat_full - obj.beta2_0);
%             obj.wald_Cheng_test_stat = obj.n * (obj.lambda_hat_full)' * inv(V_hat_full_lambda) * (obj.lambda_hat_full);
            obj.wald_Cheng_test_stat = (obj.Ns_full_lambda .* obj.lambda_hat_full)' * inv(V_hat_full_lambda) * (obj.Ns_full_lambda .* obj.lambda_hat_full);

            W = ones(length(obj.Ns_pars_lambda),1);
            Wt = sqrt(diag(V_hat_pars_lambda)).^(-1);
% %             obj.max_test_stat = sqrt(obj.n) * max(abs(W .* (obj.beta2_hat_pars - obj.beta2_0)));
% %             obj.max_t_test_stat = sqrt(obj.n) * max(abs(Wt .* (obj.beta2_hat_pars - obj.beta2_0)));
%             obj.max_test_stat = sqrt(obj.n) * max(abs(W .* (obj.lambda_hat_pars)));
%             obj.max_t_test_stat = sqrt(obj.n) * max(abs(Wt .* (obj.lambda_hat_pars)));
            obj.max_test_stat = max(abs(W .* (obj.Ns_pars_lambda .* obj.lambda_hat_pars)));
            obj.max_t_test_stat = max(abs(Wt .* (obj.Ns_pars_lambda .* obj.lambda_hat_pars)));
            
            % Simulate max test with correct covar matrix here for standard
            % critical values
                if obj.test_number == 1
                    centering_ind = [1:obj.k_delta obj.k_delta+2];
                elseif obj.test_number >= 2
                    centering_ind = [1:obj.k_delta obj.k_delta+1 obj.k_delta+2];
                end
                centering = zeros(size(obj.theta_hat_pars,1),size(obj.theta_hat_pars,2)); 
                centering(centering_ind,:) = obj.theta_hat_pars(centering_ind,:); % impose the null
                theta_sim = randn((obj.k_delta+1+2)*obj.k_beta2,1); 
                theta_sim = sqrtm(V_hat_pars) * theta_sim + centering(:);
                
%                 indicator here
                ind = ((obj.ind_lambda_pars):(obj.k_delta+1+2):(obj.k_delta+1+2)*obj.k_beta2);
...
%                 beta2_sim = randn(obj.k_beta2,1); 
%                 obj.max_t_temp = max(abs(beta2_sim))';
                 
%             type_exclude_beta = 0;
%             [~, V_hat_full_beta2_orig] = obj.fcn_V_hat(X, Y, 0, type_exclude_beta);
%             [~, V_hat_pars_beta2_orig] = obj.fcn_V_hat(X, Y, 1, type_exclude_beta);
%             obj.wald_orig_test_stat = obj.n * obj.beta2_hat_full' * inv(V_hat_full_beta2_orig) * obj.beta2_hat_full;
%             Wt_orig = sqrt(diag(V_hat_pars_beta2_orig)).^(-1);
%             obj.max_t_test_stat_orig = sqrt(obj.n) * max(abs(Wt_orig .* obj.beta2_hat_pars));   
        end
...
%                 indicator here
%                 lambda_ind = [(obj.k_delta + 1) : (obj.k_delta + obj.k_beta2)];
                lambda_ind = obj.ind_lambda_full;
                V_hat_lambda = V_hat(lambda_ind, lambda_ind);
            elseif type == 1
                A = zeros(obj.k_beta2 * (obj.k_delta+1+2), obj.n);
%                 indicator here
                lambda_ind = zeros(obj.k_beta2, 1);
                for i = 1:obj.k_beta2
                    X = [X_in(1:obj.k_delta,:); X_in(obj.k_delta+i,:); X_in(obj.k_delta+obj.k_beta2+1,:); X_in(obj.k_delta+obj.k_beta2+1+i,:)];
                    l_K = [];
                    ...
                    A(ind,:) = A_i;
                    lambda_ind(i) = (i-1) * (obj.k_delta+1+2)+obj.ind_lambda_pars;
                end
                V_hat = A * A';
                V_hat_lambda = V_hat(lambda_ind, lambda_ind);
            end
...
%                 indicator here
        ...
        function [theta_hat_full_Taylor, theta_hat_pars_Taylor, V_full, V_pars] = fcn_linearized_estimation(obj)
            ...
            X_l2 = obj.x_beta2;
            for i = 1:obj.k_beta2
                X_l(i,:) = obj.fcn_g(obj.x_beta2(i,:), 0);
                X_l2(i,:) = obj.fcn_dg(obj.x_beta2(i,:), 0);
            end
        ...
        function obj = fcn_linearized_bs(obj, M, seed)
            ...
            X_d = obj.x_delta; 
            X_d2 = obj.x_delta;
                X_d(end,:) = obj.fcn_g(obj.x_delta(end,:), obj.x_pi(1,:), 0);
                X_d2(end,:) = obj.fcn_dg(obj.x_delta(end,:), obj.x_pi(1,:), 0);
            
            % Construct the new regressors from a Taylor expansion 
            X_l = obj.x_beta2;
            X_l2 = obj.x_beta2;
            for i = 1:obj.k_beta2
                X_l(i,:) = obj.fcn_g(obj.x_beta2(i,:), obj.x_pi(1+i,:), 0);
                X_l2(i,:) = obj.fcn_dg(obj.x_beta2(i,:), obj.x_pi(1+i,:), 0);
            end
            ...
            obj.beta2_hat_full_Taylor_bs = zeros(len_l,M);
            obj.beta2_hat_pars_Taylor_bs = zeros(len_l,M);
            ...
            for m = 1:M
                ...
                %construct test stat
                beta2_hat_full_Taylor_temp = theta_hat_full_Taylor_temp(len_d+1:end);
                V_full_beta2 = V_full(len_d+1:len_d+len_l, len_d+1:len_d+len_l);
            
                beta2_hat_pars_Taylor_temp = theta_hat_pars_Taylor_temp(end,:)';
                ind = ( len_d+1 : len_d+1 : (len_d+1)*len_l );
                V_pars_beta2 = V_pars(ind, ind);
   
                Wald_Taylor_temp = obj.n * beta2_hat_full_Taylor_temp' * inv(V_full_beta2) * beta2_hat_full_Taylor_temp;
                Max_Taylor_temp = sqrt(obj.n) * max( abs( beta2_hat_pars_Taylor_temp ) )';
                Wt = sqrt( diag(V_pars_beta2) ).^(-1);
                Max_t_Taylor_temp = sqrt(obj.n) * max( abs( Wt .* beta2_hat_pars_Taylor_temp ) )';
                ...
                obj.beta2_hat_full_Taylor_bs(:,m) = beta2_hat_full_Taylor_temp;
                obj.beta2_hat_pars_Taylor_bs(:,m) = beta2_hat_pars_Taylor_temp;
            end
        ...
        %%%%%%%%%%%%%
        % All tests
        function obj = run_all_tests_fcn(obj, k_beta2_n)
            % k_beta2_n = 10; %remove this later
            obj = obj.set_params(k_beta2_n);
        ...
        function obj = max_test_stat_fcn(obj)
            obj.theta_star = zeros(obj.k_beta2_n,(obj.k_delta + 1));
            obj.y_hat = zeros(obj.n,obj.k_beta2_n);
            for i = 1:obj.k_beta2_n
        ...
        function [V, V_theta] = V_hat_fcn(obj)
            V = zeros(obj.k_beta2_n*(obj.k_delta + 1), obj.k_beta2_n*(obj.k_delta + 1));
            V_theta = zeros(obj.k_beta2_n,obj.k_beta2_n);
            for i = 1:obj.k_beta2_n
                x_i = [obj.x_delta; obj.x_theta(i, :)];
                delta_hat_i = obj.theta_star(i,1:end-1)';
                J_i = (x_i * x_i') / obj.n;
                temp_e_hat_i = obj.Y0 - delta_hat_i' * obj.x_delta;
                for j = 1:obj.k_beta2_n
        ...
        function [T_out, p_out] = max_test_pval_fcn(obj, V, M, W_in, stat_in)
            %temp_v = chol(V);
            temp_v = sqrtm(V);
            Z = randn(obj.k_beta2_n*(obj.k_delta + 1),M);
            Z_sim = temp_v * Z;
            Z_theta = zeros(obj.k_beta2_n,M);
            for i = 1:obj.k_beta2_n
        ...
        function [T_out, p_out] = max_test_bs_pval_fcn(obj, M, W_in, stat_in)
            ...
            theta_hat_wbs = zeros(M, obj.k_beta2_n);
            for m = 1:M
                temp_Y = y_wbs(m,:)';
                for i = 1:obj.k_beta2_n
                    temp_X = [obj.x_delta' obj.x_theta(i,:)'];
        ...
        function [T_star, p_val] = bs_wald_fcn(obj, wald)
            ...
            for m = 1:obj.M
                ...
                %
                temp = X_all .* repmat(e_hat, 1, obj.k_delta + obj.k_beta2_n);
                outer = ((X_all' * X_all) \ eye(obj.k_delta + obj.k_beta2_n));
                inner = temp' * temp;
                v = outer * inner * outer;
                %
                v_theta = v((obj.k_delta+1):end,(obj.k_delta+1):end);
