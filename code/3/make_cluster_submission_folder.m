...
total_num_sims = 2000; %10000;
num_sims_per_array_job = 25;
array_batch_size = 19; % number of jobs the cluster runs at one time. (the rest go in the queue.)
...
for n = T_vec
    ...
    %
    %allocate 20 min/sim (for longleaf) * J * T/100
    wall_time = 20*J*n/100;
    for dgp_type = dgp_type_vec
        job_name = sprintf('MTw%dd%dT%dJ%d', test_number, dgp_type, n, J);
        if strcmp(cluster_job_type, 'array') == 1
            m_file_name = sprintf('%s(%d,%d,%d,%d,%d,%d,%d)', sim_file_name, test_number, dgp_type, n, k_delta, k_beta2, J, seed_type);
            ...
            %
            cons_file_name = sprintf('%s(%d,%d,%d,%d,%d,%d,%d,%d)', cons_file_name, test_number, dgp_type, n, k_delta, k_beta2, J, array_start, array_end);
            fprintf(FID_cons, '%s \n \n', cons_file_name);
            %
            results_file_name = sprintf('%s(%d,%d,%d,%d,%d)', results_file_name, test_number, dgp_type, n, k_delta, k_beta2);
...
tempfile = sprintf('class_dgp_%d.m', class_number);
infile = fullfile(tempfile);
outfile = fullfile(cluster_dir, tempfile);
copyfile(infile, outfile);

tempfile = sprintf('class_tests_%d.m', class_number);
infile = fullfile(tempfile);
outfile = fullfile(cluster_dir, tempfile);
copyfile(infile, outfile);
