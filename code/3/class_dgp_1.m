classdef class_dgp_1
    properties (SetAccess=public, GetAccess=public)
     ...
     delta_0, beta_0, beta2_0, pi_0, theta_0
     lambda_0
     b1_0, b2_0
     ...
     kappa_n
     LB_pi, UB_pi
     theta_hat_full, theta_hat_pars, beta2_hat_full, beta2_hat_pars
     lambda_hat_full, lambda_hat_pars
     Ns_full, Ns_full_lambda, Ns_pars, Ns_pars_lambda
    ...
    methods
        function obj = class_dgp_1(test_number, dgp_type, hypothesis_type, n, seed_input, delta_in, beta2_in, pi_in)
            obj.test_number = test_number;
            obj.delta_0 = delta_in;
            obj.beta2_0 = beta2_in;
            obj.pi_0 = pi_in;
            %
            [obj.g_type, obj.g_type_string] = obj.fcn_set_g();
            obj.dgp_type = dgp_type;
            obj.hypothesis_type = hypothesis_type;
            obj.n = n;
%             obj.k_delta = k_delta; 
%             obj.k_beta2 = k_beta2;
            obj.k_delta = length(obj.delta_0); 
            obj.k_beta2 = length(obj.beta2_0);
...
%             obj.x_pi = randn(1+obj.k_beta2, obj.n); % the transition variable
            obj.x_pi = repmat(randn(1, obj.n),1+obj.k_beta2,1); % the transition variable
            switch dgp_type
                case 1
                    obj.dgp_type_string = 'Case 1: iid';
                    obj.x_delta = randn(obj.k_delta,obj.n);
                    obj.x_beta2 = randn(obj.k_beta2,obj.n);
                case 2 
                    obj.dgp_type_string = 'Case 2: blockwise independence';
                    w_delta = 4*randn(obj.k_delta,obj.n);
                    w_beta = 4*randn(obj.k_beta2,obj.n);
                    rho = .5; %.5;
                    num = obj.k_delta;
...
%             obj.pi_0 = zeros(obj.k_beta2+1,1);
            ...
            X_d = obj.x_delta;
            X_d(end,:) = obj.fcn_g(obj.x_delta(end,:), obj.x_pi(1,:), obj.pi_0(1));
            
            X_l = obj.x_beta2;
            for i = 1:obj.k_beta2
                X_l(i,:) = obj.fcn_g(obj.x_beta2(i,:), obj.x_pi(i+1,:), obj.pi_0(i+1));
            end
            %
            obj.Y0 = obj.delta_0' * X_d + obj.beta2_0' * X_l + obj.e;
            %
            obj.Y0 = obj.Y0((obj.init_n - obj.n + 1):end); % remove Y0 burn-in values
            %
            [obj.ind_lambda_full, obj.ind_lambda_pars] = obj.fcn_ind_lambda();
            obj.lambda_0 = obj.theta_0(obj.ind_lambda_full);
            %
        end
        function [ind_lambda_full, ind_lambda_pars] = fcn_ind_lambda(obj)
            if obj.test_number == 1
                ind_lambda_full = (obj.k_delta+1:obj.k_delta+obj.k_beta2);
                ind_lambda_pars = (obj.k_delta+1);
            elseif obj.test_number == 2
                ind_lambda_full = (obj.k_delta+obj.k_beta2+1+1:obj.k_delta+obj.k_beta2+1+obj.k_beta2);
                ind_lambda_pars = (obj.k_delta+1+1+1);
            elseif obj.test_number == 3
                ind_lambda_full = (obj.k_delta+obj.k_beta2+1+1:obj.k_delta+obj.k_beta2+1+obj.k_beta2);
                ind_lambda_pars = (obj.k_delta+1+1+1);
            elseif obj.test_number == 4
                ind_lambda_full = (obj.k_delta+obj.k_beta2+1+1:obj.k_delta+obj.k_beta2+1+obj.k_beta2);
                ind_lambda_pars = (obj.k_delta+1+1+1);
            elseif obj.test_number == 5
                ind_lambda_full = (obj.k_delta+obj.k_beta2+1+1:obj.k_delta+obj.k_beta2+1+obj.k_beta2);
                ind_lambda_pars = (obj.k_delta+1+1+1);
            elseif obj.test_number == 6
                ind_lambda_full = (obj.k_delta+obj.k_beta2+1+1:obj.k_delta+obj.k_beta2+1+obj.k_beta2);
                ind_lambda_pars = (obj.k_delta+1+1+1);
            elseif obj.test_number == 7
                ind_lambda_full = (obj.k_delta+1:obj.k_delta+obj.k_beta2);
                ind_lambda_pars = (obj.k_delta+1);
            end
        end
        function [g_type, g_type_string] = fcn_set_g(obj)
            if obj.test_number == 1
                g_type = 2; 
            elseif obj.test_number == 2
                g_type = 2; 
            ...
            if g_type == 1 
                g_type_string = 'Logistic';
            elseif g_type == 2
                g_type_string = 'Exponential';
            elseif g_type == 3
                g_type_string = 'Exponential 2';
            end
        end
        function obj = fcn_estimation_initial(obj)
            Y_in = obj.Y0;
            X_all = [obj.x_delta; obj.x_beta2; obj.x_pi];
%             % X_pi = [obj.x_delta(end,:); obj.x_beta2];
%             X_pi = repmat(obj.x_pi, 1+obj.k_beta2, 1);
%             X_pi_sorted = sort(X_pi,2);
%             obj.LB_pi = X_pi_sorted(:,floor(.15 * obj.n));
%             obj.UB_pi = X_pi_sorted(:,floor(.85 * obj.n)); 
            obj.LB_pi = -5*ones(1+obj.k_beta2, 1);
            obj.UB_pi =  5*ones(1+obj.k_beta2, 1);
            ...
            %
            obj.beta2_hat_full = obj.theta_hat_full(obj.k_delta+1:obj.k_delta+obj.k_beta2);
            obj.beta2_hat_pars = obj.theta_hat_pars(obj.k_delta+1,1:obj.k_beta2)';
            % [beta2_hat_full beta2_hat_pars]
        end
        function obj = fcn_bs2(obj, M, seed)
            ...
            theta_hat_null_imposed = obj.fcn_estimation(0, X_null, Y_in, bounds_pi); % only the full model needs to be estimated here as this is the null imposed model
                ...
                X_d(end,:) = obj.fcn_g(X_d(end,:), obj.x_pi(1,:), pi_hat_null(1));  
...
%                 e_hat_null = obj.fcn_nu_hat(theta_hat_null_imposed, X_null, Y_in, 0);
            ...
            X_all = [obj.x_delta; obj.x_beta2; obj.x_pi];
            % storage mats
            obj.theta_bs2_hat_full = zeros( obj.k_delta+1+2*obj.k_beta2, M );
            obj.theta_bs2_hat_pars = zeros( (obj.k_delta+1+1+1)*obj.k_beta2, M );
            obj.lambda_bs2_hat_full = zeros( obj.k_beta2, M );
            obj.lambda_bs2_hat_pars = zeros( obj.k_beta2, M );
            ...
            for m = 1:M
                ...
                temp_lambda_bs_hat_pars = temp_theta_bs_hat_pars(obj.ind_lambda_pars, 1:obj.k_beta2)';
                % Store the estimates
                ...
                % Cheng's Wald Stat
                obj.wald_Cheng_test_stat_bs2(m) = (obj.Ns_full_lambda .* temp_lambda_bs_hat_full)' * inv(V_hat_full_lambda) * (obj.Ns_full_lambda .* temp_lambda_bs_hat_full);
                % Max Test Stats
                W = ones(obj.k_beta2,1);
                Wt = sqrt(diag(V_hat_pars_lambda)).^(-1);
                obj.max_test_stat_bs2(m) = max(abs(W .* (obj.Ns_pars_lambda .* temp_lambda_bs_hat_pars)));
                obj.max_t_test_stat_bs2(m) = max(abs(Wt .* (obj.Ns_pars_lambda .* temp_lambda_bs_hat_pars)));
            end
        end
        %
        function out = fcn_g(obj, X1, X2, pi)
            c = 10; 
            % X1 = X; X2 = obj.x_pi;
            if obj.g_type == 1
        ...
        function out = fcn_dg(obj, X1, X2, pi)
            c = 10; 
            % X1 = X; X2 = obj.x_pi;
            if obj.g_type == 1
        ...
        function out = fcn_d2g(obj, X1, X2, pi)
            c = 10; 
            % X1 = X; X2 = obj.x_pi;
            if obj.g_type == 1
        ...
        %
        function out = fcn_g_E2(obj, X1, X2, c, pi) %E2 swaps c and pi
            out = X1 .* ( 1 - exp( -pi .* (X2 - c).^2 ) );
        end
        function out = fcn_dg_E2(obj, X1, X2, c, pi)
            out = X1 .* ( ((X2 - c).^2) .* exp( -pi .* (X2 - c).^2 ) );
        end
        function out = fcn_d2g_E2(obj, X1, X2, c, pi)
            out = - X1 .* ( ((X2 - c).^2) .* ((X2 - c).^2) .* exp( -pi .* (X2 - c).^2 ) );
        end
        %
        ...
        function nu_hat = fcn_nu_hat_full(obj, theta, X, Y, Taylor_flag)
            ...
            if num_var == obj.k_delta % for the null imposed model
                X_d = X(1:obj.k_delta,:); X_l = 0;
                delta = theta(1:obj.k_delta); beta2 = 0;
                pi = theta(obj.k_delta+1);
                %
                X_d(end,:) = obj.fcn_g(X_d(end,:), obj.x_pi(1,:), pi(1));            
                %
                nu_hat = Y - delta' * X_d - beta2' * X_l;
            elseif num_var > obj.k_delta
                X_d = X(1:obj.k_delta,:); X_l = X(obj.k_delta+1:obj.k_delta+obj.k_beta2,:);
                delta = theta(1:obj.k_delta); 
                beta2 = theta(obj.k_delta+1:obj.k_delta+obj.k_beta2);
                pi = theta(obj.k_delta+obj.k_beta2+1:end);
                %
                X_d(end,:) = obj.fcn_g(X_d(end,:), obj.x_pi(1,:), pi(1));            
                for i = 1:obj.k_beta2
                    X_l(i,:) = obj.fcn_g(X_l(i,:), obj.x_pi(i+1,:), pi(i+1));
                end
                %
                nu_hat = Y - delta' * X_d - beta2' * X_l;
            end
...
% %             X_d = X(1:obj.k_delta,:); X_l = X(obj.k_delta+1:end,:);
% %             delta = theta(1:obj.k_delta); beta2 = theta(obj.k_delta+1:obj.k_delta+obj.k_beta2);
% %             pi = theta(obj.k_delta+obj.k_beta2+1:end);
% %             %
% %             X_d(end,:) = obj.fcn_g(X_d(end,:), pi(1));            
% %             for i = 1:obj.k_beta2
% %                 X_l(i,:) = obj.fcn_g(X_l(i,:), pi(i+1));
% %             end
%             %
%             num_var = size(X,1);
%             X_d = X(1:obj.k_delta,:);
...
%         end
        function nu_hat = fcn_nu_hat_pars(obj, theta, X, Y, Taylor_flag)
            X_d = X(1:obj.k_delta,:); 
            X_l = X(obj.k_delta+1,:);
            X_p = X(obj.k_delta+1+1:obj.k_delta+1+2,:);
            delta = theta(1:obj.k_delta); 
            beta2 = theta(obj.k_delta+1);
            pi = theta(obj.k_delta+2:end);
            %
            X_d(end,:) = obj.fcn_g(X_d(end,:), X_p(1,:), pi(1));            
            i = 1;
            X_l(i,:) = obj.fcn_g(X_l(i,:), X_p(i+1,:), pi(i+1));
            %
            nu_hat = Y - delta' * X_d - beta2' * X_l;
        end
        %
        ...
        function D_nu_hat = fcn_D_nu_hat_full(obj, theta, X, type_exclude_beta, Taylor_flag)
            ...
            if num_var == obj.k_delta
                ...
                X_d(end,:) = obj.fcn_g(X_d(end,:), obj.x_pi(1,:), pi(1));            
                %
                temp1 = obj.fcn_dg(X(obj.k_delta,:), obj.x_pi(1,:), pi(1));
                %
            ...
            elseif num_var > obj.k_delta
                X_d = X(1:obj.k_delta,:); 
                X_l = X(obj.k_delta+1:obj.k_delta+obj.k_beta2,:);
                delta = theta(1:obj.k_delta); 
                beta2 = theta(obj.k_delta+1:obj.k_delta+obj.k_beta2);
                pi = theta(obj.k_delta+obj.k_beta2+1:end);
                %
                ...
                temp2 = X_l;
                for i = 1:obj.k_beta2
                    temp2(i,:) = obj.fcn_dg(X(obj.k_delta+i,:), obj.x_pi(i+1,:), pi(i+1));
                end
                %
                if type_exclude_beta == 0
                    temp1 = delta(end) * temp1;
...
% %         end
        function D_nu_hat = fcn_D_nu_hat_pars(obj, theta, X, type_exclude_beta, Taylor_flag)
            ...
            i = 1;
            X_l(i,:) = obj.fcn_g(X_l(i,:), X_p(i+1,:), pi(i+1));
            temp1 = obj.fcn_dg(X(obj.k_delta,:), X_p(1,:), pi(1));
            temp2 = obj.fcn_dg(X(obj.k_delta+1,:), X_p(i+1,:), pi(i+1));
            if type_exclude_beta == 0
                temp1 = delta(end) * temp1;
                temp2 = beta2 * temp2;
            elseif type_exclude_beta == 1
...
%         end
        ...
        function l_K_beta = fcn_l_K_beta(obj, type, l_K)
            if type == 0 % full model so all beta2s
                l_K_beta = l_K - (obj.k_delta + obj.k_beta2);
            elseif type == 1 % Pars model so only 1 beta2
        ...
        function K = fcn_K(obj, theta, X, Y, type, l_K, type_exclude_beta, theta_0_in)
            ...
            % % k_delta + k_beta2 + 1 + k_beta2.  The betas are
            % % [k_delta(end) k_delta+1:k_delta+k_beta2].
            % l_K_beta = l_K - (obj.k_beta2 + 1);
...
%             theta = obj.theta_0;
        ...
        function [mathfrac_z, V]= fcn_mathfrac_z_wrapper(obj, type, Z)
            ...
            theta_temp(obj.k_delta+1:obj.k_delta+obj.k_beta2) = zeros(obj.k_beta2,1);
            if type == 0
...
%                 theta_0_in = obj.theta_0;
                ...
                V = HG * HG';
            elseif type == 1
                mathfrac_z = zeros((obj.k_delta+1+2)*obj.k_beta2,1);
                HG = zeros((obj.k_delta+1+2)*obj.k_beta2, obj.n);
                for i = 1:obj.k_beta2
                    X = [obj.x_delta; obj.x_beta2(i,:); obj.x_pi(1,:); obj.x_pi(1+i,:)];
                    ind_theta_i = [(1:obj.k_delta) obj.k_delta+i obj.k_delta+obj.k_beta2+1 obj.k_delta+obj.k_beta2+1+i];
...
%             seed = obj.seed_input;
            ...
            %             lambda_ind_full = (obj.k_delta+1:obj.k_delta+obj.k_beta2);
            lambda_ind_full = obj.ind_lambda_full;
            obj.lambda_distr_full = obj.mathfrac_z_distr_full(lambda_ind_full,:);
            %             lambda_ind_pars = ( obj.k_delta+1 : (obj.k_delta+1+1+1) : (obj.k_delta+1+2)*(obj.k_beta2) );
            lambda_ind_pars = ( obj.ind_lambda_pars : (obj.k_delta+1+1+1) : (obj.k_delta+1+2)*(obj.k_beta2) );
            ...
            obj.test_distr_wald_Cheng = cell(obj.k_beta2,1);
            obj.test_distr_max = cell(obj.k_beta2,1);
            obj.test_distr_max_t = cell(obj.k_beta2,1);
            for k = 1:obj.k_beta2
        ...
        function l_K_out = fcn_l_K_selection(obj, theta, X, Y, type, i, type_exclude_beta)
            ...
            if type == 0 % full
                ind_beta = (obj.k_delta:obj.k_delta+obj.k_beta2);
                ind_pi = ind_beta + (obj.k_beta2 + 1);
...
%             inv_Sigma = inv(Sigma);
            ...
            if obj.test_number == 1
                temp = obj.k_delta;
            elseif obj.test_number >= 2
                temp = length(ind_beta);
            end
            for j = 1:temp % for this experiment, H_0: beta = beta2 = 0, so we put all the beta2s in l_K
                ...
                if (ICS <= obj.kappa_n) == 1
                    l_K_out = [l_K_out ind_pi(j)]; % include j's associated pi
                elseif (ICS > obj.kappa_n) == 1
                    % exclude j: do nothing here
                end
            end
            if obj.test_number == 1
                l_K_out = [l_K_out ind_pi(2:end)]; % for this experiment, H_0: beta = beta2 = 0, so we put all the beta2s in l_K
            end
        end
        %
        ...
        function theta_out = fcn_estimation_full_2(obj, type, X_in, Y_in, bounds_pi)
            ...
            LB = [repmat(-10,obj.k_delta+obj.k_beta2,1); bounds_pi(:,1)]; UB = [repmat(10,obj.k_delta+obj.k_beta2,1); bounds_pi(:,2)];
        ...
        function theta_out = fcn_estimation_pars_2(obj, type, X_in, Y_in, bounds_pi)
            theta_init = [obj.delta_0; obj.beta2_0; obj.pi_0];
            LB_all = [repmat(-10,obj.k_delta+obj.k_beta2,1); bounds_pi(:,1)]; UB_all = [repmat(10,obj.k_delta+obj.k_beta2,1); bounds_pi(:,2)];
            for i = 1:obj.k_beta2
                ind_pi = [1 1+i]; 
                ind_pars = [1:obj.k_delta obj.k_delta+i obj.k_delta+obj.k_beta2+ind_pi];
        ...
        function theta_out = fcn_estimation_full(obj, type, X_in, Y_in, bounds_pi)
            ...
            elseif num_var > obj.k_delta
                ...
                for k = 1:length(obj.pi_0)
                    R(k, obj.k_delta+obj.k_beta2+k) = 1;
                end
            end
            % R = [0 0 1 0; 0 0 0 1]; 
            R = [R; -R];
        ...
        function theta_out = fcn_estimation_pars(obj, type, X_in, Y_in, bounds_pi)
            theta_out = zeros(obj.k_delta+1+2, obj.k_beta2);
            LB = bounds_pi(:,1); UB = bounds_pi(:,2);
            for i = 1:obj.k_beta2
                % i=1; 
                ind_pi = [1 1+i]; 
                ...
                % X_pars = [X_in(1:obj.k_delta,:); X_in(obj.k_delta+i,:)];
                X_pars = [X_in(1:obj.k_delta,:); X_in(obj.k_delta+i,:); X_in(obj.k_delta+obj.k_beta2+1,:); X_in(obj.k_delta+obj.k_beta2+1+i,:)];
...
%             type = 0; 
            ...
            theta_in = theta_init; gamma_in = 1;
            f_in = obj.fcn_nu_hat(theta_in, X, Y, type);
            Df_in = obj.fcn_D_nu_hat(theta_in, X, type);
            while (it < 20) % (check > tol && it < 20)
                [theta_out, f_out, Df_out, gamma_out] = obj.fcn_NLLS(theta_in, gamma_in, Df_in, f_in, X, Y, type, R, c);
                check = abs(theta_in - theta_out)' * abs(theta_in - theta_out);
                it = it + 1;
                theta_in = theta_out; f_in = f_out; Df_in = Df_out; gamma_in = gamma_out;
            end
        end
        function [theta_out, f_out, Df_out, gamma_out] = fcn_NLLS(obj, theta_in, gamma_in, Df_in, f_in, X, Y, type, R, c)
            % Levenberg-Marquardt Algorithm
            A = ( Df_in * Df_in' + (gamma_in .* eye(size(Df_in,1)) ) ) \ eye(size(Df_in,1));
            B = Df_in * f_in';
            ...
            if norm_f_out < norm_f_in
                gamma_out = .9 * gamma_in;
            elseif norm_f_out >= norm_f_in
                gamma_out = 2 * gamma_in;
...
%             theta_in = theta_out; f_in = f_out; Df_in = Df_out;
        ...
        function [B_full, B_pars] = fcn_B(obj, theta_full, theta_pars)
            B_full = [ones(obj.k_delta+obj.k_beta2,1); abs(theta_full(obj.k_delta:obj.k_delta+obj.k_beta2))];
            num = (obj.k_delta+1+1+1);
            B_pars = ones( num*obj.k_beta2,1);
            ind = [];
            for temp_k = 1:obj.k_beta2
                temp = num*(temp_k-1) + (obj.k_delta+1+1:num);
                ind = [ind temp];
            end
            ind2 = ind-(obj.k_delta+1);
            B_pars(ind) = abs(theta_pars(ind2));
        end
        %
    end % end methods
end % end class
