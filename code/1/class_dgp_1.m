        ...
        function obj = class_dgp_1(dgp_type, hypothesis_type, n, seed_input, k_lambda, k_delta, beta_in)
            ...
            switch dgp_type
                case 1
                    ...
                    obj.x_lambda = randn(obj.k_lambda,obj.n);
                case 2 
...
%             obj.k_delta = obj.k_delta+1; % and increase k_delta to account for the intercept
            ...
            obj.lambda_0 = zeros(obj.k_lambda,1);
            obj.lambda_0(1) = beta_in(2);
            ...
            for i = 1:obj.k_lambda
                X_l(i,:) = obj.fcn_g(obj.x_lambda(i,:), obj.pi_0(i+1));
            end
            
            obj.Y0 = obj.delta_0' * X_d + obj.lambda_0' * X_l + obj.e;
...
%             % X_pi = [obj.x_delta(end,:); obj.x_lambda];
...
%             obj.UB_pi = X_pi_sorted(:,floor(.85 * obj.n)); 
            ...
            %
            obj.lambda_hat_full = obj.theta_hat_full(obj.k_delta+1:obj.k_delta+obj.k_lambda);
            obj.lambda_hat_pars = obj.theta_hat_pars(obj.k_delta+1,1:obj.k_lambda)';
...
%                 e_hat_null = obj.fcn_nu_hat(theta_hat_null_imposed, X_null, Y_in, 0);
        ...
        function nu_hat = fcn_nu_hat_full(obj, theta, X, Y, Taylor_flag)
            ...
            if num_var == obj.k_delta % for the null imposed model
                ...
                delta = theta(1:obj.k_delta); lambda = 0;
                pi = theta(obj.k_delta+1);
                %
                X_d(end,:) = obj.fcn_g(X_d(end,:), pi(1));            
                %
                nu_hat = Y - delta' * X_d - lambda' * X_l;
            elseif num_var > obj.k_delta
                X_d = X(1:obj.k_delta,:); X_l = X(obj.k_delta+1:end,:);
                delta = theta(1:obj.k_delta); lambda = theta(obj.k_delta+1:obj.k_delta+obj.k_lambda);
...
%                 lambda = theta(obj.k_delta+1:obj.k_delta+obj.k_lambda);
%                 pi = theta(obj.k_delta+obj.k_lambda+1:end);
%                 X_d(end,:) = obj.fcn_g(X_d(end,:), pi(1));
%                 for i = 1:obj.k_lambda
%                     X_l(i,:) = obj.fcn_g(X_l(i,:), pi(i+1));
...
%         end
        function nu_hat = fcn_nu_hat_pars(obj, theta, X, Y, Taylor_flag)
            X_d = X(1:obj.k_delta,:); X_l = X(obj.k_delta+1,:);
            delta = theta(1:obj.k_delta); lambda = theta(obj.k_delta+1);
        ...
        function D_nu_hat = fcn_D_nu_hat_full(obj, theta, X, type_exclude_beta, Taylor_flag)
            ...
            elseif num_var > obj.k_delta
                ...
                if type_exclude_beta == 0
                    temp1 = delta(end) * temp1;
                    temp2 = repmat(lambda, 1, obj.n) .* temp2;
                elseif type_exclude_beta == 1
                    %do nothing
                end
...
%             theta = obj.theta_0;
        ...
        function [mathfrac_z, V]= fcn_mathfrac_z_wrapper(obj, type, Z)
            ...
            theta_temp(obj.k_delta+1:obj.k_delta+obj.k_lambda) = zeros(obj.k_lambda,1);
            if type == 0
...
%             inv_Sigma = inv(Sigma);
            ...
            l_K_out = [l_K_out ind_pi(2:end)]; % for this experiment, H_0: beta = lambda = 0, so we put all the lambdas in l_K
        end
        %
        ...
        function theta_out = fcn_estimation_pars_2(obj, type, X_in, Y_in, bounds_pi)
            ...
            for i = 1:obj.k_lambda
                ...
                theta_init = [obj.delta_0; obj.lambda_0(i); obj.pi_0(ind_pi)];
        ...
        function theta_out = fcn_estimation_pars(obj, type, X_in, Y_in, bounds_pi)
            ...
            for i = 1:obj.k_lambda
                ...
                % X_pars = [obj.x_delta(end,:); obj.x_lambda(i,:)];
                X_pars = [X_in(1:obj.k_delta,:); X_in(obj.k_delta+i,:)];
